<?php
  ...
  if (isset($_POST['saveSetting'])) {
    $wprotation_lucky = get_option('woo_rotation_lucky_setting');
    if ($wprotation_lucky) {
      update_option('woo_rotation_lucky_setting',$_POST['woo_rotation_lucky_setting']);
    } else {
      add_option('woo_rotation_lucky_setting',$_POST['woo_rotation_lucky_setting']);
    }
    $successMessage = 'Lưu cài đặt thành công';
  }

  if (isset($_POST['saveSettingLevel2'])) {
    $wprotation_luckyLevel2 = get_option('woo_rotation_lucky_setting_cap2');
    if ($wprotation_luckyLevel2) {
      update_option('woo_rotation_lucky_setting_cap2',$_POST['woo_rotation_lucky_setting_cap2']);
    } else {
      add_option('woo_rotation_lucky_setting_cap2',$_POST['woo_rotation_lucky_setting_cap2']);
    }
    $successMessage = 'Lưu cài đặt thành công';
  }
  if (isset($_POST['saveSettingRewardPolicy'])) {
    $wprotation_luckyLevel2 = get_option('reward_policy');
    if ($wprotation_luckyLevel2) {
...
<div class="wrap">
  ...
  <?php } ?>
  <ul class="nav-tabs-rotation_lucky">
  ...
  <div class="tab-content">
    <div id="tab-setting-1-content" class="tab-pane-rotation_lucky active">
      <?php require_once(dirname(__FILE__) . '/user-list.php'); ?>
    </div>
    <div id="tab-setting-2-content" class="tab-pane-rotation_lucky">
      <form action="?page=hoa-hong&paged=1&tab=setting2" method="POST">
        <h4>Cài đặt cấp 1</h4>
        <input type="number" max="100000000" class="regular-text" name="woo_rotation_lucky_setting" value="<?php echo get_option('woo_rotation_lucky_setting'); ?>" />
        <button type="submit" class="button button-primary" name="saveSetting">Lưu lại</button>
      </form>
      <form action="?page=hoa-hong&paged=1&tab=setting2" method="POST">
        <h4>Cài đặt cấp 2</h4>
        <input type="number" max="100000000" class="regular-text" name="woo_rotation_lucky_setting_cap2" value="<?php echo get_option('woo_rotation_lucky_setting_cap2'); ?>" />
        <button type="submit" class="button button-primary" name="saveSettingLevel2">Lưu lại</button>
      </form>
    </div>
    <div id="tab-setting-3-content" class="tab-pane-rotation_lucky">
      <?php require_once(dirname(__FILE__) . '/transfer.php'); ?>
    </div>
    <div id="tab-setting-4-content" class="tab-pane-rotation_lucky">
      <?php require_once(dirname(__FILE__) . '/revenue-statistics.php'); ?>
    </div>
    <div id="tab-setting-5-content" class="tab-pane-rotation_lucky">
